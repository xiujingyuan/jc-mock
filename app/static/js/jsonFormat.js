...
var jsonsh = {
    ...
    init: function () {
        ...
        jQuery('#source_args').keyup(function () {
            if (jQuery("#text-content").hasClass('active')) {
                return false;
            }
            /** Only process JSON if there is some, and it is not the same as before */
        ...
        jQuery('#source_replace').keyup(function () {
            if (jQuery("#text-content").hasClass('active')) {
                return false;
            }
            /** Only process JSON if there is some, and it is not the same as before */
    ...
    type_change: function (type) {
        if (type === 1) {
            ...
            $('#result_args').hide().html('');
            $('#source_args').focus();
            return false;
        }
        else if (type === 5) {
            $('#result_replace').hide().html('');
            $('#source_replace').focus();
            return false;
        }
        else if (type === 7) {
            $('#result_expect').hide().html('');
            $('#source_expect').focus();
            return false;
        }
        jsonsh.make_pretty(type);


    },

    /**  */
    make_pretty: function (type) {
        ...
        if (type == 4){
            source_id = '#source_args';
        }
        else if (type == 6){
            source_id = '#source_replace';
        }
        else if (type == 8){
            source_id = '#source_expect';
        }
        if (jQuery(source_id).val() != '') {
            try {
                /** Parse JSON using Awesome JSON Lint */
                var result = jsonlint.parse(jQuery(source_id).val());
                ...
                if (result) {
                    ...
                    if (jsonsh.is_pretty === false) {
                        ...
                        jQuery(source_id).val(JSON.stringify(result, null, jsonsh.tab_space)).scrollTop(0);
                    ...
                    if (type === 2) {
                        ...
                        jQuery('#result_args').fadeOut(jsonsh.animation_speed);
                    }
                    else if (type === 6) {
                        jQuery('#result_replace').fadeOut(jsonsh.animation_speed);
                    }
                    else if (type === 8) {
                        jQuery('#result_expect').fadeOut(jsonsh.animation_speed);
                    }
                ...
                if (type === 2) {
                    ...
                    jQuery('#result_args').addClass('fail').html(error.message).fadeIn();
                }
                else if (type === 6) {
                    jQuery('#result_replace').addClass('fail').html(error.message).fadeIn();
                }
                else if (type === 8) {
                    jQuery('#result_expect').addClass('fail').html(error.message).fadeIn();
                }
